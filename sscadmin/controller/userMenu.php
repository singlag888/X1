 ...
 * 控制器：权限菜单管理
 */
class userMenuController extends sscAdminController
{
    //方法概览
    public $titles = array(
        'menuList' => '权限列表',
    ...
    public function menuList()
    {
            ...
            switch ($sa) {
                case 'saveSort':
                    ...
                    foreach ($sort_ids as $menu_id => $sort) {
                        userMenus::updateItem($menu_id, array('sort' => $sort));
                    }
                    showMsg("保存成功");
                    break;
            }
        ...
        $menus = userMenus::getItems($parent_id);

        self::$view->setVar('menus', $menus);
        self::$view->setVar('parent_id', $parent_id);
        self::$view->setVar('prev_id', $prev_id);
        self::$view->setVar('actionLinks', array(0 => array('title' => '增加权限', 'url' => url('userMenu', 'addMenu', array('parent_id' => $parent_id)))));
        self::$view->render('usermenu_menulist');
    }
    ...
    public function addMenu()
    {
        ...
        $locations  = array(0 => array('title'=>'返回权限列表','url'=>url('userMenu','menuList', array('parent_id' => $parent_id))));
        //新增数据
        ...
        $firstSecondParents = userMenus::getAllLevelItems(0, 1);
        self::$view->setVar('firstSecondParents', $firstSecondParents);
        self::$view->setVar('parent_id', $parent_id);
        self::$view->render('usermenu_addmenu');
        $GLOBALS['mc']->flush();
    }
    ...
    public function editMenu()
    {
        ...
        $locations  = array(0 => array('title'=>'权限列表','url'=>url('userMenu','menuList', array('parent_id' => $prev_id))));
        ...
        if ($this->request->getPost('submit', 'trim')) {
            ...
            if (!userMenus::updateItem($this->request->getPost('menu_id', 'intval'), $data)) {
        ...
        $menu = userMenus::getItem($menu_id);
    ...
    {
        $locations  = array(0 => array('title'=>'返回权限列表','url'=>url('userMenu','menuList')));
        if (!$menu_id = $this->request->getGet('menu_id', 'intval')) {
            showMsg("参数无效", 1, $locations);
        }

        if (userMenus::getItems($menu_id)) {
            showMsg("下面有子菜单，不能删除，若要删除，请先删除子菜单", 0, $locations);
        }

        if (!userMenus::deleteItem($menu_id)) {
