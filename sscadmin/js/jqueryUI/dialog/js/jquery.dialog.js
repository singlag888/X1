...
(function($) {
    $.fn.dialog = function(settings, wnd) {
        ...
        $('body', wnd.document).append('<div id="ui-dialog" style="outline: 0px none; z-index: 9002;" class="ui-dialog" tabindex="-1"><div class="ui-dialog-titlebar"><span class="ui-dialog-title" id="ui-dialog-title"></span></div><div class="ui-dialog-content"></div><div class="ui-dialog-button ui-helper-clearfix"><div id="ui-dialog-buttonset" class="ui-dialog-buttonset"></div></div></div>');
        ...
        $('body', wnd.document).append('<div id="ui-widget-overlay" class="ui-widget-overlay" style="z-index: 9001;"></div>');
...
//再封装一下 默认在父窗口弹出，如需在本窗口弹出，需加第三个参数window.self
    $.alert = function(msg, callback, wnd) {
        var wnd = wnd || window.parent;
        $('<div title="提示">' + msg + '</div>').dialog({
            bgiframe: true,
            width: 300,
            buttons: {
                "确定": function() {
                    $(this).dialog("close", wnd);       //关闭这个对话框
    ...
    $.confirm = function(msg, callback, wnd) {
        var wnd = wnd || window.parent;
        $('<div title="提示">' + msg + '</div>').dialog({
            bgiframe: true,
            width: 300,
