...
class domain
{
    ...
    {
            $sql = 'SELECT a.*,b.name ,c.user_id, c.nick_name , c.ext_code,c.is_test parent_is_test , c.type parent_type FROM user_domains a , domains b ,users c WHERE a.username = c.username AND  a.domain_id = b.domain_id ';
            $paramArr = array();
            if ($domain_id != 0) {
...
//           echo $sql;exit;
    ...
    static public function getUsername($username,$order,$start = -1,$amount = DEFAULT_PER_PAGE){//根据用户名查询
    	$sql = 'SELECT a.*,c.name , b.nick_name , b.ext_code ,b.user_id FROM user_domains a , users b , domains c WHERE  b.username = a.username AND a.domain_id = c.domain_id';
    	if (!empty($username)){
    		$sql .= " AND b.username = "."'".$username."'";
    		if (!empty($order)){
    			$sql .= " ORDER BY a.ud_id ".$order;
     ...
     * @author L
     */
    static public function getUdidDomain($ud_id,$ext_code){//根据ud_id获取列表的单条信息
    	$sql = "SELECT  a.*,c.name , b.nick_name , b.ext_code FROM user_domains a , users b , domains c WHERE  c.domain_id = a.domain_id and a.username = b.username";
    	if(!empty($ud_id)){
     ...
     * @author L
     */
    static public function getUdidDomainC($ud_id){//根据ud_id获取列表的单条信息
        $sql = "SELECT  a.*,c.name , b.nick_name  FROM user_domains a , users b , domains c WHERE  c.domain_id = a.domain_id and a.username = b.username";
        if(!empty($ud_id)){
            $sql .= " AND a.ud_id = "."'".$ud_id."'";
        }
    ...
    static public function domainsAdd($top_id,$domain_id,$username){	
    	...
    	if(!isset($level['level'])){
    		showMSG("您输入的代理帐号不是网站会员");
    	}
    	if($level['level'] !== 0){
    		showMSG("您输入的代理帐号不是总代理，请到：用户管理->'代理管理'页面确认");
    	}
    ...
    static public function usersUpdata($nick_name,$username){//修改users表的昵称和推广码

        if (!empty($nick_name)  && !empty($username)){
            $userSql = "UPDATE users SET nick_name = "."'".$nick_name."'WHERE username = "."'".$username."'";
        }
...
//    	if($res == 0){
//    		showMSG("请确认输入的推广码长度是否在6到8之间，且是否为字母，数字或字母数字组合");
//    	}
