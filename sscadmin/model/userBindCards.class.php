...
class userBindCards
{
    static public function getItem($id = 0, $user_id = 0, $status = 1)
    {
        $sql = 'SELECT * FROM user_bind_cards WHERE 1';
        if (strlen($id) >= 15) {
            $sql .= " AND card_num = '$id'";
        }
        else {
    ...
    static public function getItems($user_id = 0, $bank_id = 0, $card_num = '', $status = -1, $start = -1, $amount = DEFAULT_PER_PAGE, $username='', $bank_username='')
    {
        $sql = 'SELECT * FROM user_bind_cards WHERE 1';
        if($user_id != 0){
            $sql .= " AND user_id = {$user_id}";
