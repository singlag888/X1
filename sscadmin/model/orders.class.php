...
class orders
{
    ...
    static public function getItemsExclude($options = [])
    {
        ...
        $sql =<<<SQL
SELECT a.*,b.username,b.status AS user_status,b.type AS user_type 
...
SQL;
        ...
        if ($default_options['lottery_id'] != 0) {
            $sql .= " AND a.lottery_id = " . intval($default_options['lottery_id']);
        }
        if ($default_options['issue'] != '') {
            $sql .= " AND a.issue = '{$default_options['issue']}'";
        }
        if (!empty($default_options['type'])) {
            if (is_array($default_options['type'])) {
                $sql .= " AND a.type IN(" . implode(',', $default_options['type']) . ")";
            }
            else {
                $sql .= " AND a.type = " . intval($default_options['type']);
            }
        }
        if ($default_options['user_id'] != '') {
            ...
            if (!$default_options['include_childs']) {
                ...
                if($default_options['freeze'] == -1) {
                    ...
                    // $teamUsers = users::getUserTree($default_options['user_id'], true, 1, 8,-1,'',1);
                    $teamUsers = users::getUserTreeField([
                        'field' => ['user_id'],
                        'parent_id' => $default_options['user_id'],
                        'recursive' => 1,
        ...
        if ($default_options['start_amount'] > 0) {
            $sql .= " AND abs(a.amount) >= {$default_options['start_amount']}";
        }
        if ($default_options['end_amount'] > 0) {
            $sql .= " AND abs(a.amount) <= {$default_options['end_amount']}";
        }

        if ($default_options['start_time'] != '') {
            $sql .= " AND a.create_time >= '{$default_options['start_time']}'";
        }
        if ($default_options['end_time'] != '') {
            $sql .= " AND a.create_time <= '{$default_options['end_time']}'";
        }
        $sql .= ' ORDER BY order_id DESC';
    ...
    static public function getItemsExcludeStatistics($options = [])
    {
        ...
        $default_options = is_array($options) ? array_merge($default_options, $options) : $default_options;
        $sql =<<<SQL
SELECT LEFT(a.create_time, 10) AS belongDate,a.type, SUM(amount) AS amount,COUNT(DISTINCT (a.from_user_id)) AS num 
FROM orders AS a,users AS b
WHERE a.from_user_id = b.user_id 
...
SQL;
        ...
        $sql .= ' GROUP BY belongDate, a.type DESC';
        $result = $GLOBALS['db']->getAll($sql);
        return $result;
    }

    /**
     * author snow 修改使用数组传参数
     * @param array $options
     * @return mixed
     */
    static public function getTrafficInfoExclude($options = [])
    {
...
// }
        ...
        if($freeze == -1) {
            ...
            $allChilds = array_keys(users::getItems($user['user_id'], false, true, array('user_id'),8,-1,'','','','','','','','','','',-1,-1,DEFAULT_PER_PAGE,[],'',-1,1));
...
//        $result = "T{$lottery_id}{$str}E";
    ...
    //批量插入提高速度-后台使用
    static public function addItemsAdmin($datas)
    {
        if (!is_array($datas)) {
            throw new exception2('参数无效', 3409);
        }
        ...
        $tmp = array();
        foreach ($datas as $v) {
            $tmp[] = "('{$v['lottery_id']}','{$v['issue']}','{$v['package_id']}','{$v['from_user_id']}','{$v['from_username']}','{$v['to_user_id']}','{$v['to_username']}','{$v['type']}','{$v['amount']}','{$v['pre_balance']}','{$v['balance']}','{$v['create_time']}','{$v['business_id']}','{$v['admin_id']}','{$v['remark']}')";
        }
        $sql = "INSERT INTO orders (lottery_id,issue,package_id,from_user_id,from_username,to_user_id,to_username,`type`,amount,pre_balance,balance,create_time,business_id,admin_id,remark) VALUES " . implode(',', $tmp);
    ...
    static public function getOrderInfoByCond($where = '',$field = '*',$orderBy = ' order_id desc')
    {
       $sql = "select $field from orders where ".$where.' ORDER BY '.$orderBy;
       return  $GLOBALS['db']->getRow($sql);
    }
}
