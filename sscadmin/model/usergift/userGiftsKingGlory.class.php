...
class userGiftsKingGlory extends userGiftsBase implements userGiftsInerface
{
    const STATUS_LOSE = 2;
    const STATUS_WIN = 3;
    ...
    private $type = 600; // 帐变类型 与 最大非中奖基数 统计Type // 礼品卷中也是按这个分类的
    private $maxCost = 0; // 最大非中奖基数 // 计算方式 50次中一次就是 50 * cost
    private $winRate = 0;
    private $lockKey = 'user_gift_king_glory_lock_';
    private $cacheKey = 'king_glory_hero_poker_pic';
    private $userId = 0;
    ...
    function __construct($settings)
    {
        ...
        $this->cost = config::getConfig('king_glory_cost', 2);
        $this->maxCost = config::getConfig('king_glory_max_cost', 50 * $this->cost);
        $this->winRate = config::getConfig('king_glory_win_rate', 0.1);
        $this->winRate > 1 && $this->winRate = 1;
        $this->winRate < 0 && $this->winRate = 0;
        $this->heroPokerPicDir = ROOT_PATH . 'sscmobile/images/mobile/king_glory/skin/';
    }
...
//        }
    ...
    public function createMobileBanner()
    {
        ...
        if (isset($GLOBALS['SESSION']['user_id'])) {
            $html = '<li><a href="?c=user&a=userGifts&giftType=king_glory" class="lb-img1 fl"><img src="' . $cdnUrl . '/images/mobile/king_glory/top_banner.png"/></a></li>';
        } else {
            $html = '<li><a href="javascript:;" class="goLogin"><img src="' . $cdnUrl . '/images/mobile/king_glory/top_banner.png"/></a></li>';
        }
    ...
    public function runFrontLogicControll($request, $view)
    {
        ...
        if (IS_AJAX) {
            ...
            /* 其余操作就离开此处 */
            $option = $request->getPost('option', 'trim', '');
            if ($option && method_exists($this, $method = 'custom' . ucfirst($option))) {
                $this->$method($request, $view);
            }
        ...
        $view->render('user_gifts_king_glory');
        exit;
    }
     ...
     * @var redisCache $GLOBALS ['redis']
     */
    private function customHistory($request, $view)
    {
        $where = [
            'user_id' => $this->userId,
            'status' => ['IN', [self::STATUS_WIN, self::STATUS_FINISH]],
            'title' => $this->cnTitle
        ];

        $userGifts = M('user_gifts');

        $count = $userGifts->where($where)->count();

        if ($count) {
            $page = $request->getPost('page', 'intval', 1);
            $page <= 0 && $page = 1;
            $limit = $request->getPost('limit', 'intval', 10);
            $limit <= 0 && $limit = 10;
            $offset = ($page - 1) * $limit;

            $data = $userGifts
                ->field('`ug_id`,`remark` AS `title`,`to_time`,`status`')
                ->where($where)
                ->order('ug_id DESC')
                ->limit($offset, $limit)
                ->select();
