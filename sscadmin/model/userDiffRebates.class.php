...
//logdump($sql);
    ...
    //共享库后用这个方法取
    static public function getRecentNoRebates2($recentNoRebateId, $start = -1, $amount = DEFAULT_PER_PAGE)
    {
        if (!is_numeric($recentNoRebateId)) {
            throw new exception2('参数无效', 3320);
        }
        $sql = "SELECT udr.* FROM `user_diff_rebates` udr LEFT JOIN check_send cs ON udr.lottery_id=cs.lottery_id AND udr.issue=cs.issue WHERE 1" .
                " AND udr.status = 0 AND cs.status_check_prize = 2 AND udr.udr_id >= $recentNoRebateId";
        $sql .= " ORDER BY udr.udr_id";
        if ($start > -1) {
            $sql .= " LIMIT $start, $amount";
        }
    ...
    static public function getTeamRebates($user_id, $is_test = -1, $lottery_id = 0, $modes = 0, $start_time = '', $end_time = '',$domain = '',$freeze = -1)
    {
        if($freeze == -1) {
            ...
            # TODO : is_test 默认是 -1, 这里传的0, 暂不清楚具体逻辑
