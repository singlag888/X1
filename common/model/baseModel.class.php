...
class baseModel
{
    ...
    public function select($option = [])
    {
        ...
        isset($this->options['limit']) && $sql .= $this->options['limit'];
        $index = isset($this->options['index']) ? $this->options['index'] : '';

        # TODO : 更多的查询连接条件  distinct union lock comment force

        $sql = $this->formatSql($sql, self::METHOD_SELECT);

        $swoole = isset($this->options['swoole']) ? !!$this->options['swoole'] : true;
        $this->options = [];
        if ($option === false) {
            return [$sql, $PARAM];
        } else if ($option === true) {
            return $this->parseBind($sql, $PARAM);
        }

        $this->recordSql($sql, $PARAM);
        return $this->db->getAll($sql, $PARAM, $index, $swoole);
    }
     ...
     */
    public function insert($data = [], $option = [], $replace = false)
    {
        ...
        $valStr = '(' . implode(',', array_keys($PARAM)) . ')';
        $sql = ($replace ? 'REPLACE' : 'INSERT') . ' INTO ' . $this->parseTable() . ' ' . $fields . ' VALUES ' . $valStr;
        ...
        $this->recordSql($sql, $PARAM);
        $result = $this->db->query($sql, $PARAM, 'i');
     ...
     * @return int | array | bool $insert_id
     */
    public function insertAll($dataSet = [], $option = [], $replace = false)
    {
        in_array($dataSet, [false, true], true) && $option = $dataSet;
        (is_bool($dataSet) || empty($dataSet)) && $dataSet = $this->data;
        $this->data = [];
        ...
        foreach ($dataSet as $index => $data) {
            ...
            // 将所有的key添加上 ':',并拼接上索引号
            foreach ($data as $val) {
                $name = count($this->options['bind']);
                $value[] = ':' . $index . '_' . $name;
                $this->bind(':' . $index . '_' . $name, $val);
            }

            $values[] = '(' . implode(',', $value) . ')';
        }

        $fields = '(`' . implode('`,`', array_keys($dataSet[0])) . '`)';
        $sql = ($replace ? 'REPLACE' : 'INSERT') . ' INTO ' . $this->parseTable() . ' ' . $fields . ' VALUES ' . implode(',', $values);
        $sql = $this->formatSql($sql, self::METHOD_INSERT);

        $PARAM = $this->options['bind'];
        $this->options = [];
        if ($option === false) {
            return [$sql, $PARAM];
        } else if ($option === true) {
    ...
    public function update($data = [], $option = [])
    {
        ...
        $sql = 'UPDATE `' . $this->parseTable() . '` SET ';
    ...
    /**
     * set limit parameter by current page and page size
     * @param int $page
     * @param int $listRows
     * @return $this
     */
    public function page($page = 1, $listRows = 20)
    {
        $page = $page > 0 ? $page : 1;
        $listRows = $listRows > 0 ? $listRows : (is_numeric($this->options['limit']) ? $this->options['limit'] : 20);
        $offset = ($page - 1) * $listRows;
        $this->options['limit'] = ' LIMIT ' . $offset . ',' . $listRows;
        return $this;
    }
    ...
    protected function parseWhere($conditions = [])
    {
        ...
        if (isset($this->options['whereStr'])) {
            $sql .= ($sql ? ' AND ' : ' ') . $this->options['whereStr'] . ' ';
        }
     ...
     * @param $PARAM
     */
    protected function recordSql($sql, $PARAM = [])
    {
        $sql = $this->parseBind($sql, $PARAM);
     ...
     * @return string
     */
    protected function parseBind($sql, $PARAM = [])
    {
        return strtr($sql, array_map(function ($val) {
    ...
    public function sql($index = false)
    {
        if ($index === true) {
            ...
            return $this->sqlCount() > 0 ? end($GLOBALS['_sql']) : '';
        }
    }
