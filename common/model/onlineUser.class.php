...
class onlineUser
{
    ...
    {
        $this->stepLength = defined('ONLINE_EXPIRATION_TIME') ? ONLINE_EXPIRATION_TIME : 1200;
    }
    ...
    private function recordOnlineHash($userId)
    {
        ...
        $GLOBALS['redis']->expire($oneStepKey, $this->stepLength * 4);

        // 记录2步键
        $GLOBALS['redis']->hSetNx($twoStepKey, $userId, 1);
        $GLOBALS['redis']->expire($twoStepKey, $this->stepLength * 4);
    }
    ...
    {
        $totalStep = 86400 / $this->stepLength; // 一定要可以整除
        $hour = date('G', REQUEST_TIME);
        $minute = date('i', REQUEST_TIME);
        $right = floor(($hour * 3600 + $minute * 60) / $this->stepLength) + 1 + $offset;
