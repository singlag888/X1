...
//            if (!empty($mobile) && $mobile != $user['mobile']) $data['mobile'] = $mobile;
        ...
        //>>author snow 添加支行名称
        $card = userBindCards::getItemByUid($user_id,1,'bank_id,card_num,bank_username,branch');
        ...
        $data = [
            ...
            'bank_name' => $bank_name,
            //>>author snow 添加支行名称
            'branch' => isset($card['branch']) ? $card['branch'] : '',
            'total_win'=>round($today_selfWinReport['total_win'],3)
        ];

        $this->showMsg(0, mobileErrorCode::RETURN_SUCCESS, $data);
    }

        public function getUserTransferAmount()
    {

        list($user_id, $user) = $this->chkUser();
        $mw = new MW();
        //>>author snow 原来的数据是这样,不想改
        $response = json_decode($mw->userInfo($user_id, $user['parent_id'], $user['top_id'], $user['username']),true);
        $response['balance'] = $user['balance'];
        if (isset($response['msg'])) {
            $this->showMsg(1, $response['msg'],$response);
        } else {
            if (isset($response['money'])){

                $this->showMsg(0, mobileErrorCode::RETURN_SUCCESS,$response);
            } else {
                $this->showMsg(2, '未知错误');
            }
        }
    ...
    {
        $time=time();
        $e_time=explode('_',$key);
        if(count($e_time)!=2) return false;
        if(empty($e_time[0]) || $e_time[0]<$time) $this->showMsg('7034','验证码已过期,请重新获取');
    ...
    private function _handleRegRebate($var, $regNeedMobile, $realNameToggle, $mobile, $realname)
    {
        ...
        } elseif($register_from == 'app'){//app不填推广吗归属到ID最小的推广总代
            $topUser = M()->table('users')
                            ->field([
                                'user_id',
                            ])
                            ->where([
