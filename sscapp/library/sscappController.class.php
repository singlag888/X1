...
class sscappController extends controller
{
    ...
    private function getSetSessionToken()
    {
        if (!(CONTROLLER == 'user' && ACTION == 'login')) {
            if ($this->request->getGet('is_wap', 'intval', 0) == 3) {
    ...
    protected function showMsg($errno, $errstr, $data = '')
    {
        ...
        if (!empty($data)) {
            ...
            if (empty($token)) {
                $result = ['errno' => 7032, 'errstr' => '缺少apiToken'];
            } else {
     ...
     * @return mixed
     */
    public function cutRedisDatabase(Closure $closure,$db_index=REDIS_DB_APP)
    {
     ...
     * @param string $logName
     */
    public function logdumpNew($logName = '支付', $logContent)
    {
        static $count = 0;
        $str = '';
        if (extension_loaded('xdebug')) {
        ...
        if (is_string($logContent)) {
            ...
            $str .= var_export($logContent, true) . "\n";
        }
    ...
    //>>今日个人盈亏
    protected function getTodaySelfWinReport($user_id){
        $s_today=date('Y-m-d').' 00:00:00';
        $e_today=date('Y-m-d').' 23:59:59';
        return $this->count_selfWinReport($user_id,$s_today, $e_today);
    }

    //>>个人盈亏和投注金额统计
    protected function selfWinReport($user_id,$start_time,$end_time){
        $today_selfWinReport = $this->getTodaySelfWinReport($user_id);
        $time_ago = $end_time < date('Y-m-d 00:00:00') ? true : false;
        if(!$time_ago){
            $end_time=date('Y-m-d 23:59:59', strtotime('-1 days'));
        }
        $selfWinReport= $this->cutRedisDatabase(function()use($user_id,$start_time,$end_time,$time_ago)
        {
            $cacheExpire = CACHE_EXPIRE_LONG;
            $key = 'selfWinReport_'.$user_id.'_'.strtotime($start_time).strtotime($end_time);
            return redisGet($key, function () use($user_id,$start_time,$end_time,$time_ago){
                return $this->count_selfWinReport($user_id,$start_time, $end_time,$time_ago);
            },$cacheExpire);
        });
        if(!$time_ago) {
            $selfWinReport["total_amount"] = $selfWinReport["total_amount"] + $today_selfWinReport["total_amount"];
            $selfWinReport["total_win"] = $selfWinReport["total_win"] + $today_selfWinReport["total_win"];
        }
        return [
            'today'=>$today_selfWinReport,
            'all'=>$selfWinReport,
        ];
    }
    /*
        ...
        */
    protected function count_selfWinReport($user_id,$start_time, $end_time,$write_cache_flag = false)
    {
        $cacheKey = __FUNCTION__ . 'getReport' . $user_id . '_' . $start_time . '_' . $end_time;
        //给每一次不同查询条件缓存一次，做到程序优化和数据库减压
        if ($write_cache_flag) {
            $total_count=$this->cutRedisDatabase(function()use($user_id,$start_time, $end_time,$cacheKey){
                $total_count=$GLOBALS['redis']->get($cacheKey);
                if($total_count===false){
                    $total_count=projects::getReportByUid($user_id, $start_time, $end_time);
                    $GLOBALS['redis']->setex($cacheKey,CACHE_EXPIRE_LONG,serialize($total_count));
                }else $total_count=unserialize($total_count);
                return $total_count;
            });
        } else  $total_count=projects::getReportByUid($user_id, $start_time, $end_time);

        $self = array();
        //获取自己数据
        $self['total_amount'] = round(isset($total_count['total_amount']) ?$total_count['total_amount'] : 0,3);
        $self['total_prize']= round(isset($total_count['total_prize']) ? $total_count['total_prize'] : 0,3);
        $self['total_win'] = round(($self['total_prize'] - $self['total_amount']), 4);
        return $self;
    }
}
