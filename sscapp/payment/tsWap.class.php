 ...
 * 通扫支付
 * @author L
 * @time 2017年11月30日 12:41:06
 * @group 支付小组
 */
namespace sscapp\payment;

class tsWap extends payModel
{
    private $_partner = '';//商户ID
    private $_callbackurl = ''; //异步回掉地址
    private $_key = '';//商户key
    private $_returnurl = '';//商户key
    private $_requestUrl = 'http://i.kldgz.com/18/wapPay';//第三方请求地址
    ...
    public function run($data = [])
    {
        ...
        if (is_array(json_decode($res,true))) {
            ...
            if ($arrBack['respCode'] !== '00') {
                ...
                $this->logdump('tsWap', $arrBack['message']);
                return false;
            }
        }else{
            echo 1;die;
    ...
    public function callback($response = [], $is_raw = 0)
    {
        ...
        if (empty($response)) {
            $this->logdump('tsWap', '回掉异常(第三方返回参数为空,请支付小组查看处理!)');
            exit;
        }
        if ($is_raw) {
        ...
        if ($response['signature'] == $sign)//签名正确
        {
            ...
            $this->bak('gs4fj@5f!sda*dfuf', 'SUCCESS', $deposits, $sysnumber, 'tsWap');
        } else {
            $this->logdump('tsWap', '回掉(签名验证不通过!请支付小组查看!)');
            exit;
        }
        die('ok');//到不了这里
    }
