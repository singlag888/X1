...
class articles
{
    ...
    /******************************** snow 获取部分字段的新列表 start*************************************************/

    static public function getItemsFields($category_id = -1, $status = -1, $start = -1, $amount = DEFAULT_PER_PAGE,$fields = '*')
    {
        //>>处理得到需要查询的字段
        $select = '';
        if (is_string($fields))
        ...
        $sql = 'SELECT '. $select .',c.name as category_name FROM articles a LEFT JOIN article_categories c ON a.category_id = c.category_id WHERE 1';
        if ($category_id != -1) {
            if (preg_match('`^\d+$`', $category_id)) {
                $sql .= " AND c.category_id = " . intval($category_id);
            }
        ...
        $sql .= ' ORDER BY a.is_stick DESC, a.create_time DESC, a.article_id DESC';
        if ($start > -1) {
            $sql .= " LIMIT $start, $amount";
        }
        $result = $GLOBALS['db']->getAll($sql);
