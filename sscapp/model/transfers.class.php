...
//活动的结束日期,用于判断一些逻辑是否进入运行
    ...
    static public function transferInMW($user_id, $transferAmount)
    {
        ...
        $transferPayResult = $mw->transfer($transferPrepareResult['asinTransferOrderNo'], $transferPrepareResult['asinTransferDate'], $transferAmount, $transferOrderNo);
    ...
    static public function transferOutMW($user_id, $transferAmount)
    {
        ...
        $orderData = array(
            ...
            'to_user_id' => 0,
            'to_username' => '',
            'type' => 162, // 帐变类型 101充值 102手续费优惠 103首存优惠 104再存优惠 105其他优惠 106提现不符退款 161从波音转出 162从电子转出 201提现 211转入波音 301返点 302下级返点 303撤单返款 304追号中止返款 308中奖 321理赔 401投注 411撤消返点 412撤单手续费 413撤消中奖
            'amount' => $transferAmount,
            'pre_balance' => $user['balance'],
            'balance' => round($user['balance'] + $transferAmount, 4),
            'create_time' => date('Y-m-d H:i:s'),
