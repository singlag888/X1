...
class users
{
    ...
    static public function updateBasicInfoById($data,$uid){
        $tmp='';
        foreach ($data as $k=>$val)if(!is_array($val)&&!is_object($val))$tmp[]=$k.'='.$val;
        if(!empty($tmp)){
            $sql = "UPDATE users SET " . implode(',', $tmp) . " WHERE user_id=$uid LIMIT 1";
            if ($GLOBALS['db']->query($sql, array(), 'u')) return true;
        }
        return false;
    }
    ...
    static public function getItem($id, $status = 8, $needLock = false, $flag = 1,$freeze = -1, $field = '*')
    {
        ...
        if ($status != -1) {
            if($freeze == -1) {
                ...
                $sql .= ' AND status = 1 and status = ' . intval($status);
            }
        }
...
//		$sql = "SELECT u.*, (SELECT s.expire_time FROM sessions s WHERE u.user_id = s.user_id ORDER BY s.expire_time DESC LIMIT 1) expire_time FROM users u WHERE 1";
        ...
        $sql = "SELECT b.username AS parent_name,b.is_test as parent_is_test,b.type as parent_type,u.* FROM users u LEFT JOIN users as b ON u.parent_id = b.user_id WHERE 1";
    ...
    static public function getItemsByLike($parent_id = -1, $includeSelf = true, $recursive = 0, $fields = array(), $status = 8, $is_test = -1, $startDate = '', $endDate = '', $lastStartDate = '', $lastEndDate = '', $username = '', $reg_ip = '', $real_name = '', $mobile = '', $qq = '', $email = '', $has_deposited = -1, $orderBy = [], $start = -1, $amount = DEFAULT_PER_PAGE,$select='u.*')
    {

//		$sql = "SELECT u.*, (SELECT s.expire_time FROM sessions s WHERE u.user_id = s.user_id ORDER BY s.expire_time DESC LIMIT 1) expire_time FROM users u WHERE 1";
        $sql = "SELECT ".$select." FROM users u WHERE 1";
        if ($parent_id != -1) {
            if ($recursive) {
                $sql .= " AND (FIND_IN_SET('$parent_id', u.parent_tree)";
            } else {
    ...
    static public function getOnlineItemsById($online,$parent_id = -1, $includeSelf = true, $recursive = 0, $fields = array(), $status = 8, $has_deposited = -1, $orderBy = [], $start = -1, $amount = DEFAULT_PER_PAGE,$select='u.*')
    {
...
//		$sql = "SELECT u.*, (SELECT s.expire_time FROM sessions s WHERE u.user_id = s.user_id ORDER BY s.expire_time DESC LIMIT 1) expire_time FROM users u WHERE 1";
        ...
        $sql.=" AND u.user_id IN (".implode(',',$online).")";
        if ($orderBy && is_array($orderBy)) {
            foreach ($orderBy as $orderName => $by) {
                $sql .= " ORDER BY u." . $orderName;
                $sql .= $by == -1 ? ' DESC' : ' ASC';
    ...
    static public function getAllOnlineItemsById($online,$parent_id = -1, $includeSelf = true, $recursive = 0,$select='u.*',$status=8)
    {
...
//		$sql = "SELECT u.*, (SELECT s.expire_time FROM sessions s WHERE u.user_id = s.user_id ORDER BY s.expire_time DESC LIMIT 1) expire_time FROM users u WHERE 1";
    ...
    //得到某个用户团队余额
    static public function getTeamBalance($user_id, $include_self = true, $is_test = -1)
    {
    ...
    //得到所有总代团队余额
    static public function getTopTeamBalance($status = -1, $is_test = -1)
    {
    ...
    static public function login($username, $pwd, $secpwd = '', $first_flag = 2, $frm = 1)
    {
        ...
        if ($user) {
            ...
            if ($GLOBALS['mc']->get('lock', $user_id)) {
                $flag = -3;
                $str = '由于您今日已有4次输入密码错误，帐号已被锁定24小时，请明天再来，或联系客服解锁';
            } else {
                if ($user['status'] != 8) {
                    $flag = -3;
                    $str = '您的帐户存在异常，请联系上级';
        ...
        if ($flag == 0) {
            ...
            if (!($rs=$GLOBALS['db']->query($sql, array(), 'u')) && $rs!=0) {
                $flag = -4;
...
//冻结该用户成功后冻结银行卡
    ...
    static public function manualUpdateBalanceBatch($params, $userIdArr, $orderType, $admin_id)
    {
        ...
        foreach ($params as $k => $v) {
            $tmp = $v['balance'] + $v['amount'];
            $wh .= " WHEN {$v['user_id']}  THEN $tmp ";
            $va .= "(0,'',{$v['user_id']},'{$v['username']}',0,'',$orderType,{$v['amount']},{$v['balance']},{$tmp},'{$timeTmp}',0,$admin_id,'{$v['remark']}'),";
        }
        $va = rtrim($va, ',');
     ...
     */
    static public function transferMoney($fromUser, $toUser, $amount, $safePwd)
    {
        if ($amount <= 0) {
            throw new exception2('参数无效');
        }
        ...
        if ($fromUser['safe_pwd'] != generateEnPwd($safePwd)) {
            $GLOBALS['db']->rollback();
            throw new exception2('您输入的安全码不对');
        }
     ...
     * @return bool
     */
    static public function checkSafePwd($userPwd, $inputPwd, $user_id = null, $lose = false)
    {
        ...
        if (!$userPwd) {
            return '您尚未设置安全码，请先<a href=\'?c=user&a=editSafePwd\'>点此设置安全码</a>';
        }
