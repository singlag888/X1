...
class notices
{
    ...
    /**
     * author snow
     * 修改,使用redis缓存 使用options 传参
     * @param $options
     * @return array|bool|mixed
     */
    static public function getItems($options)
    {
        ...
        $expired    = $default_options['expired'];
        $start      = $default_options['start'];
        $amount     = $default_options['amount'];
        ...
        $result     = redisHashGet($hashKey,$cacheKey,function () use ($status, $start, $expired, $amount, $type, $field){
            $sql = 'SELECT ' . $field . ' FROM notices WHERE 1';
            if ($status !== NULL) {
