...
class withdraws
{
    ...
    static public function getItems($user_id = '', $include_childs = 0, $is_test = -1, $status = -1, $pay_bank_id = 0, $pay_card_id = 0, $bank_id = 0, $startDate = '', $endDate = '', $start = -1, $amount = DEFAULT_PER_PAGE, $create_stime = '', $create_etime = '',$freeze = -1)
    {
    ...
    static public function new_getItems($user_id = '', $include_childs = 0, $status = -1, $start = -1, $amount = DEFAULT_PER_PAGE, $create_stime = '', $create_etime = '',$orderby='a.withdraw_id DESC')
    {
        $sql = 'SELECT a.withdraw_id,b.user_id,b.username,a.create_time,a.finish_time,a.amount,a.status,a.remark FROM withdraws a LEFT JOIN users b ON a.user_id=b.user_id WHERE 1';
        //尽量用user_id为优
        if ($user_id != '') {
            $tmp = preg_match('`^\d+$`Ui', $user_id);
            if (!$include_childs) {
...
//            $sql .= " AND a.finish_time <= '$create_etime'";
    ...
    static public function getTrafficInfo($user_id = '', $include_childs = 0, $is_test = -1, $status = -1, $pay_bank_id = 0, $pay_card_id = 0, $bank_id = 0, $startDate = '', $endDate = '', $create_stime = '', $create_etime = '',$freeze = -1)
    {
    ...
    static public function getTeamWithdraws($user_id, $startDate = '', $endDate = '',$freeze = -1)
    {
        ...
        if($freeze == -1) {
            $sql = 'SELECT w.user_id,u.parent_id, u.parent_tree, SUM(w.amount) AS total_withdraw FROM withdraws w LEFT JOIN users u ON w.user_id=u.user_id WHERE u.status = 8 AND u.is_test = 0 AND w.status = 8';
        }else{
            $sql = 'SELECT w.user_id,u.parent_id, u.parent_tree, SUM(w.amount) AS total_withdraw FROM withdraws w LEFT JOIN users u ON w.user_id=u.user_id WHERE (u.status = 8 or u.status = 1) AND u.is_test = 0 AND w.status = 8';
        }
        if ($startDate != '') {
    ...
    static public function applyWithdraw($user_id, $withdraw_bank_id, $withdraw_card_num, $province, $city, $branch_name, $withdraw_amount, $secpwd = '', $bank_name = '')
    {
        ...
        if ($withdraw_amount > $config['max_withdraw_limit']) {
            return "金额超过当日提款限额{$config['max_withdraw_limit']}，无法提交";
        }
    ...
    //前台用户申请提款  写帐变扣余额，若不符再加条“取消提款”的帐变并退还游戏币
    static public function _new_applyWithdraw($user_id, $withdraw_bank_id, $withdraw_card_num, $province, $city, $branch_name, $withdraw_amount, $secpwd = '', $bank_name = '')
    {
        if (!isset($GLOBALS['cfg']['withdrawBankList'][$withdraw_bank_id])) return "请选择要提款的银行";
        if (!$withdraw_card_num) return "请输入卡号";
        $config = config::getConfigs(array('min_withdraw_limit', 'max_withdraw_limit','max_withdraw_day_limit',''));

        if ($withdraw_amount < $config['min_withdraw_limit']) {
            return "金额不足最低提款限额{$config['min_withdraw_limit']}，无法提交";
        }
        /*********************** 修改文案 限制单笔金额不能多于100万  单日限额不能大于1000万 **********************/
        if ($withdraw_amount > $config['max_withdraw_limit']) {
        ...
        /***********************  添加 限制单笔金额不能多于100万 单日限额不能大于1000万 ***************/
        //开始事务
        $GLOBALS['db']->startTransaction();
