...
class game
{
    ...
    {
        if (!is_numeric($lottery_id) || !preg_match('`^[\d-]{6,14}$`', $issue) || !is_numeric($curRebate) || $curRebate < 0 || !is_numeric($modes)  || !is_numeric($user_id) || !is_array($projects) || !is_numeric($multiple) || $multiple <= 0 || !is_numeric($xgame)) {
    ...
    {
        if (!is_numeric($lottery_id) || !preg_match('`^[\d-]{6,14}$`', $issue) || !is_numeric($curRebate) || $curRebate < 0 || !is_numeric($modes)  || !is_numeric($user_id) || !is_array($projects)) {
            log2("errcode=999", func_get_args());
            throw new exception2('参数不对', 999);
        }
    ...
    {
        if (!is_numeric($lottery_id) || !is_numeric($curRebate) || $curRebate < 0 || !is_numeric($modes) || !is_numeric($user_id) || !is_array($projects) || empty($projects) || !is_array($traceData) || empty($traceData) || $stopOnWin < 0 || !strlen($curIssue)) {
            log2("errcode=999", func_get_args());
            throw new exception2('参数不对', 999);
        }
        //1.先检查彩种是否在售，是否在销售期，奖金组是否存在
        ...
        if(!isset($traceData[0]) || !isset($traceData[0]['issue']))
        {
            throw new exception2('参数不对1', 999);
        }
    ...
    static public function batchCheckPrize($lottery, $issueInfo)
    {
        ...
        if ((!$lottery = lottery::getItem($lottery['lottery_id'])) || $lottery['status'] != 8) {
            throw new exception2('彩种正在维护中，稍后开放', 1120);
        }
    ...
    static public function checkPrize($lottery, $issueInfo, $packages)
    {
        ...
        $methods = methods::getItemsNew(['method_id', 'name'],$lottery['lottery_id']);
        //得到该期所有游戏用户的返点
        ...
        foreach ($packages as $package) {
            ...
            foreach ($packageProjects[$package['package_id']] as $v) {
                ...
                if (md5($v['user_id'] . $v['code'] . $v['create_time']) != $v['hash_value']) {
                    log2("system halted.号码已被非法修改，拒绝进行中奖判断 project_id:{$v['project_id']},user_id:{$v['user_id']},code:{$v['code']}");
    ...
    static public function cancelTrace($user_id, $trace_id, $package_ids, $admin_id = 0)
    {
        ...
        //>>author snow 把锁定操作,移到这一步,防止出现死锁.
    ...
    static public function mmcBuy($curRebate, $modes, $projects, $multiple, $openCounts = 1)
    {
        ...
        //////////////////////// 购彩前 校验 ///////////////////////////////
        if (!is_numeric($lottery_id) || !is_numeric($curRebate) || $curRebate < 0 || !is_numeric($modes)  || !is_numeric($user_id) || !is_array($projects) || !is_numeric($multiple) || $multiple <= 0) {
            log2("errcode=999", func_get_args());
            throw new exception2('参数不对', 999);
        }
