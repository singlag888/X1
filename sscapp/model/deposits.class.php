...
class deposits
{
    ...
    //$is_manual 是否手动充值
    static public function getItems($user_id = '', $include_childs = 0, $is_test = -1, $is_manual = -1, $status = -1, $trade_type = 0, $deposit_bank_id = 0, $deposit_card_id = 0, $start_amount = 0, $end_amount = 0, $order_num = '', $startDate = '', $endDate = '', $start = -1, $amount = DEFAULT_PER_PAGE, $create_stime = '',$create_etime = '',$orderby='deposit_id DESC',$freeze = -1)
    {
    ...
    static public function new_getItems($user_id = '', $include_childs = 0, $status = -1, $startDate = '', $endDate = '', $start = -1, $amount = DEFAULT_PER_PAGE, $orderby='deposit_id DESC')
    {
        $sql = 'SELECT a.deposit_id,b.user_id,b.username,a.create_time,a.finish_time,a.amount,a.status,a.trade_type FROM deposits a LEFT JOIN users b ON a.user_id=b.user_id WHERE 1';
        if ($user_id != '') {
            $tmp = preg_match('`^\d+$`Ui', $user_id);
            if (!$include_childs) {
                if ($tmp) {
...
//            $sql .= " AND a.finish_time <= '$endDate'";
    ...
    static public function getTrafficInfo($user_id = '', $include_childs = 0, $is_test = -1, $is_manual = -1, $status = -1, $trade_type = 0, $deposit_bank_id = 0, $deposit_card_id = 0, $start_amount = 0, $end_amount = 0, $order_num = '', $startDate = '', $endDate = '', $create_stime = '',$create_etime = '',$freeze = -1)
    {
...
//        $result = "T{$lottery_id}{$str}E";
    ...
    static public function getTeamDeposits($user_id, $startDate = '', $endDate = '',$freeze = -1)
    {
        if($freeze == -1) {
            // $teamUsers = users::getUserTree($user_id, true, 1, 8);
            $teamUsers = users::getUserTreeField([
                'field' => ['user_id', 'parent_id', 'level', 'parent_tree'],
                'parent_id' => $user_id,
                'recursive' => 1,
                'status' => 8,
            ]);
        ...
        if($freeze == -1) {
            ...
            $sql = 'SELECT d.user_id,u.parent_id, u.parent_tree, SUM(d.amount) AS total_deposit FROM deposits d LEFT JOIN users u ON d.user_id=u.user_id WHERE (u.status = 8 or u.status = 1) AND u.is_test = 0 AND d.status = 8';
...
//        }
    ...
    {
        $GLOBALS['db']->query('SET SESSION group_concat_max_len=1024000; ');
        $sql = 'SELECT count(*) AS total_count,sum(amount) AS total_amount  FROM (';
        $sql .= 'SELECT a.deposit_id,b.real_name FROM deposits a LEFT JOIN users b ON a.user_id=b.user_id WHERE 1';

        //>>添加排除统计优惠彩金
        $sql .= ' AND a.trade_type != 6';
        //>>添加排除总代dcsite 相关数据
        $sql .= ' AND b.top_id !=IF((SELECT user_id FROM users WHERE username = \'dcsite\'),(SELECT user_id FROM users WHERE username = \'dcsite\'),-1)';
        if ($username != '') {
            if ($include_childs == 0) {
                $sql .= " AND b.username = '$username'";
            }
