...
class miBei extends newPayModel
{
    ...
    public function send_post($url, $post_data)//POST传输
    {
        $options = array(
            'http' => array(
                'method' => 'POST',
    ...
    public function run($data=[])
    {
        ...
        if ($code['result_code'] = 'SUCCESS' && !empty($code['sign'])){
            ...
            if (\pay::addItem($array)) {
                if (strstr($data['card_info']['netway'], "WAP") > '-1') {
                    if (isset($code['prepay_url'])){
                    header("location:" . $code['prepay_url']);
                    die();
                    }else{
                        echo '通道繁忙，请稍后支付';
                        $this->logdump('mbPay',$code);
                    }
                } else {
            ...
            echo '系统繁忙，请稍后访问';
            $this->logdump('mbPay', $postData);
            $this->logdump('mbPay', $res);
        }
    }
