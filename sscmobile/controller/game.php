...
class gameController extends sscController
{
    ...
    public function play()
    {
        ...
        switch ($op) {
            case 'buy':
                $lottery_id = $this->request->getPost('lotteryId', 'intval');
                ...
                try {
                    // Optimized 参数项 codes by Well
                    $codes = game::getCodesType($codeStr);
                    if ($traceData) { //追号情况
                        if ($lottery_id == 15) {
                            throw new exception2('非法访问');
                        }
                        $trace_id = game::trace($lottery_id, $curRebate, $modes, $GLOBALS['SESSION']['user_id'], $codes, $traceData, $stopOnWin, $issue);
                        $wrap_id = traces::wrapId($trace_id, $issue, $lottery_id);
                    } else {
                        if ($lottery_id == 15) {
                            ...
                            //中间有任何异常抛出
                            $package_id = game::buy($lottery_id, $issue, $curRebate, $modes, $GLOBALS['SESSION']['user_id'], $codes, $multiple, $xgame);
                            $wrap_id = projects::wrapId($package_id, $issue, $lottery_id);
                        }
                    }
...
//                break;
    ...
    {
        $projects = array();
        if ($tmp = projects::getItems($lotteryId, $issue, 0, $GLOBALS['SESSION']['user_id'], date('Y-m-d 00:00:00'), date('Y-m-d 23:59:59'), 0, 5)) {
    ...
    {
        $lottery_id = 16;
        $template_name = 'game_115_x';  //暂没有区分
        self::_show($lottery_id, $template_name, methodGroups::GROUP_CREDIT);
    }

    public function bjpksIframe()
    {
        $lottery_id = 17;
        $template_name = 'game_pks_iframe';
        self::_show($lottery_id, $template_name, methodGroups::GROUP_OFFICIAL);
    }
...
//        $startPos = ($this->request->getGet('curPage', 'intval', 1) - 1) * 9;

        /******************************* 修改查询时间为2周以前。********************************/
       // $this->searchDate($start_time, $end_time, 2, 3);
        $start_time = date('Y-m-d 00:00:00', strtotime('-60 days'));
        ...
        if ($wrap_id) {
            ...
            $startPos = getStartOffset($curPage, $packagesNumber, 9);//>>为什么分页是9
            /***************** snow  获取正确的分页开始****************************/

            $packages = projects::getPackages($lottery_id, $check_prize_status == 65535 ? -1 : $check_prize_status, -1, '', -1, 0, $GLOBALS['SESSION']['user_id'], $include_childs, $start_time, $end_time, '', '', $cancel_status, '', $startPos, 9);
        ...
        self::$view->setVar('pageList', getPageListMobile($packagesNumber, 9));
        if ($mmcPopup == '1') {
...
//        $startPos = ($this->request->getGet('curPage', 'intval', 1) - 1) * DEFAULT_PER_PAGE;
        ...
        self::$view->setVar('totalPrize', $totalPrize);
        self::$view->setVar('lotterys', $lotterys);
        self::$view->setVar('pageList', getPageListMobile($packagesNumber, DEFAULT_PER_PAGE));
...
//        $this->searchDate($start_time, $end_time, 15, 1);
        $start_time = date('Y-m-d 00:00:00', strtotime('-14 days'));
        $end_time = date('Y-m-d 23:59:59');
        /******************************* 修改查询时间为2周以前。********************************/
        ...
        if ($wrap_id) {
            ...
            /***************** snow  获取正确的分页开始****************************/
            $curPage  = $this->request->getGet('curPage', 'intval', 1);
            $startPos = getStartOffset($curPage, $tracesNumber, 9); //>>为里为什么又是9
            /***************** snow  获取正确的分页开始****************************/

            $traces = traces::getItems($lottery_id, $is_test, $issue, $modes, $GLOBALS['SESSION']['user_id'], $include_childs, $start_time, $end_time, $startPos, 9, $status);
...
//                            $totalProfit += $traces[$k]['profit'];
        ...
        self::$view->setVar('pageList', getPageListMobile($tracesNumber, 9));
    ...
    public function chart()
    {
        ...
        self::$view->setVar('fun', $fun[$lottery_id]);
        switch ($lottery['lottery_type']) {
            case 3:
