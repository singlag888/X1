...
class userController extends sscController
{
    ...
    public function userGifts()
    {
        ...
        $giftType = $this->request->getGet('giftType', 'trim', '');

        $giftType or $giftType = $this->request->getPost('giftType', 'trim', '');
        //活动前端控制逻辑
...
//        $startPos = ($this->request->getGet('curPage', 'intval', 1) - 1) * DEFAULT_PER_PAGE;
        $curPage = $this->request->getGet('curPage', 'intval', 1) ;//>>snow获取当前页
        ...
        if ($user = users::getItem($username)) {
            if ($user['user_id'] == $GLOBALS['SESSION']['user_id']) {
                ...
                foreach ($allParent as $parent) {
                    ...
                    if ($is_children) {
                        $userLevelName .= ' &gt; <a href="/?c=user&a=childList&username=' . $parent['username'] . '&range=' . $range . '">' . $parent['username'] . '</a>';
                    }
                }
        ...
        $todayRegCount = $teamBalance = $teamTotal = 0;
        ...
        if ($flag == 1) {
            ...
            $allUsers = users::getItemsByLike($user['user_id'], true, $range);
            $teamTotal = count($allUsers);
            /***************** snow  获取正确的分页开始****************************/
            //>>snow 不想改别人写的东西.只解决分页bug
            $startPos  = getStartOffset($curPage, $teamTotal);
            /***************** snow  获取正确的分页开始****************************/
            $users = users::getItemsByLike($user['user_id'], true, $range, array(), 8, -1, '', '', '', '', '', '', '', '', '', '', -1, $orderBy, $startPos, DEFAULT_PER_PAGE);
            if ($allUsers) {
                foreach ($allUsers as $agent) {
                    $teamBalance += $agent['balance'];
                    if ($agent['reg_time'] >= date('Y-m-d 00:00:00')) $todayRegCount += 1;
                }

                $onlineUsers = (new onlineUser())->getIntersectByUid(array_keys($allUsers));
            }
            ...
            foreach ($users as $iUserId => $value) {
                ...
                $users[$iUserId]['online'] = in_array($iUserId, $onlineUsers) ? 1 : 0;
                if ($online != $users[$iUserId]['online'] && $online != -1) {
                    unset($users[$iUserId]);
                }
            }
        }
        ...
        self::$view->setVar('todayRegCount', $todayRegCount);
        self::$view->setVar('countOfOnline', count($onlineUsers));
        self::$view->setVar('loginUser', $GLOBALS['SESSION']);
        self::$view->setVar('username', $username);
        self::$view->setVar('range', $range);
        self::$view->setVar('online', $online);
        self::$view->setVar('flag', $flag);
        self::$view->setVar('users', $users);
        self::$view->setVar('pageList', getPageList(count($allUsers)));
...
//            $totalInfo['first_deposit_amount'] += $v['first_deposit_amount'];
    ...
    public function childReport()
    {
        ...
        if ($flag == 1) {
            $teamDeposit = deposits::getTeamDeposits($user['user_id'], $startDate, $endTime,1);
            $teamWithdraw = withdraws::getTeamWithdraws($user['user_id'], $startDate, $endTime,1);
            $teamBalance = users::getTeamBalances($user['user_id'],1);

            //先得到自己和直接下级
            $users = users::getItems($user['user_id'], true, 0, array('user_id', 'balance', 'last_time'), -1,-1,'','','','','','','','','','',-1,-1,DEFAULT_PER_PAGE,[],'',1,1);
            ...
            $childReport = projects::getChildReport($user['user_id'], $startTime, $endTime,1);
            ...
            foreach ($childReport as $k => $v) {
                ...
                $totalInfo['pt_game_win'] += 0;
                $totalInfo['pt_prize'] += 0;    //PT中奖
                $totalInfo['pt_buy_amount'] += 0;
                $totalInfo['pt_amount'] += 0;
            }
        ...
        foreach ($childReport as $k => $v) {
            ...
            $childReport[$k]['user_inactive_days'] = isset($users[$user_id]['inactive_days'])?$users[$user_id]['inactive_days']:'';
            ...
            $childReport[$k]['pt_game_win'] = 0;
            $childReport[$k]['pt_prize'] = 0;   //PT中奖
            $childReport[$k]['pt_buy_amount'] = 0;
            $childReport[$k]['pt_amount'] = 0;
        }
        ...
        $selfUser = users::getItem($GLOBALS['SESSION']['user_id'],8,false,1,1);
        $selfUser['daySalary'] = $selfUser['weekSalary'] = $selfUser['monthSalary'] = 0;
        $salaryStandard = config::getConfigs(array('day_standard', 'week_standard', 'month_standard'));
        if ($selfUser['level'] == 0) {//如果是总代才查询日工资和月工资
            if ($selfUser['day_salary'] != 0) {
                $yesterday = date('Y-m-d', strtotime($endDate) - 86400);
                $yesterdayTimeS = "{$yesterday} 00:00:00";
                $yesterdayTimeE = "{$yesterday} 23:59:59";
                $daySalary = projects::getTeamSaleC($user_id, $yesterdayTimeS, $yesterdayTimeE, 0,1);
                if (!empty($daySalary)) {
                    if ($daySalary[$user_id] >= $salaryStandard['day_standard']) {
                        $selfUser['daySalary'] = $daySalary[$user_id]['total_amount'] * $selfUser['day_salary'];
                    }
                }
            }
            if ($selfUser['week_salary'] != 0) {
                $lastSunday = date('Y-m-d 23:59:59', strtotime('last sunday'));
                $lastMonday = date('Y-m-d 00:00:00', strtotime($lastSunday) - 86400 * 6);
                $weekSalary = projects::getTeamSaleC($user_id, $lastMonday, $lastSunday, 0,1);
                if (!empty($weekSalary)) {
                    if ($weekSalary[$user_id] >= $salaryStandard['week_standard']) {
                        $selfUser['weekSalary'] = $weekSalary[$user_id]['total_amount'] * $selfUser['week_salary'];
                    }
                }
            }
            if ($selfUser['month_salary'] != 0) {
                $lastMonthS = date('Y-m-01 00:00:00', strtotime('-1 month'));
                $lastMonthE = date('Y-m-t 23:59:59', strtotime('-1 month'));
                $monthSalary = projects::getTeamSaleC($user_id, $lastMonthS, $lastMonthE, 0,1);
                if (!empty($monthSalary)) {
                    if ($monthSalary[$user_id] >= $salaryStandard['day_standard']) {
                        $selfUser['monthSalary'] = $monthSalary[$user_id]['total_amount'] * $selfUser['month_salary'];
                    }
                }
        ...
        self::$view->setVar('totalInfo', $totalInfo);
        self::$view->setVar('users', $users);
        self::$view->setVar('selfUser', $selfUser);
...
//        $startPos = ($this->request->getGet('curPage', 'intval', 1) - 1) * DEFAULT_PER_PAGE;
        ...
        if ($username != $GLOBALS['SESSION']['username']) {
            //根据自己的权限列出可查找的用户
            // $GLOBALS['SESSION']['user_id'], true, 1, 8
...
//        $startPos = ($this->request->getGet('curPage', 'intval', 1) - 1) * DEFAULT_PER_PAGE;
        ...
        if ($user = users::getItem($username)) {
            ...
            /***************** snow  获取正确的分页开始****************************/
            $startPos = getStartOffset($curPage, $userDiffRebatesNumber);
            /***************** snow  获取正确的分页开始****************************/
        ...
        self::$view->setVar('users', $users);
        self::$view->setVar('pageList', getPageList($userDiffRebatesNumber, DEFAULT_PER_PAGE));
        //self::$view->setVar('actionLinks', array(0 => array('title' => '增加用户', 'url' => url('user', 'addUser'))));
...
//        $startPos = ($this->request->getGet('curPage', 'intval', 1) - 1) * DEFAULT_PER_PAGE;
        ...
        $this->searchDate($start_time, $end_time,2,3);
...
//            $packagesTotal['total_profit'] = 0;
        ...
        if ($packages) {
            ...
            foreach ($packages as $k => $v) {
                ...
                if(!isset($lotterys[$v['lottery_id']]))
                {
                    $packages[$k]['prize_mode'] = '未知';
                }else {
...
//        $startPos = ($this->request->getGet('curPage', 'intval', 1) - 1) * DEFAULT_PER_PAGE;
        ...
        self::$view->setVar('pageList', getPageList($messagsNumber, DEFAULT_PER_PAGE));
...
//        $startPos = ($this->request->getGet('curPage', 'intval', 1) - 1) * DEFAULT_PER_PAGE;
        ...
        if ($flag == 1) {
            ...
            /************************** snow  团队实时报表只展示一个团队的, 所以不用分页start************************************************/
//            $usersNumber = users::getItemsNumber($user['user_id'], true, 0, 8);
            $usersNumber = 1;
            /************************** snow  团队实时报表只展示一个团队的, 所以不用分页start************************************************/

            //这里包含了PT数据 需询问产品是否去除或者接入
            $teamTotalProjects = projects::getChildReport($user['user_id'], date('Y-m-d 00:00:00'), date('Y-m-d H:i:s'),1);
            $teamTotalDeposits = deposits::getTeamDeposits($user['user_id'], date('Y-m-d 00:00:00'), date('Y-m-d H:i:s'),1);
            $teamTotalWithdraws = withdraws::getTeamWithdraws($user['user_id'], date('Y-m-d 00:00:00'), date('Y-m-d H:i:s'),1);
            $teamTotalGifts = userGifts::getTeamTotalGift($user['user_id'], date('Y-m-d 00:00:00'), date('Y-m-d H:i:s'),1);
            ...
            $pageData['page_promo_active'] = $self['total_promo_active'] = isset($teamTotalGifts[$user['user_id']])?$teamTotalGifts[$user['user_id']]:0; //彩票活动暂时保留

            // $pageData['page_win'] = $self['total_win'] = round(($self['total_prize'] + $self['total_rebate'] + $self['total_promo_active'] - $self['total_amount']), 4);
            $pageData['page_win'] = $self['total_win'] = round(($self['total_prize'] + $self['total_rebate'] - $self['total_amount']), 4);
            ...
            foreach ($users as $user_id => $v) {
                if(isset($teamReport[$user_id])) {
                    ...
                    $teamReport[$user_id]['total_promo_active'] = isset($teamTotalGifts[$user_id])?$teamTotalGifts[$user_id]:0;
                     if(isset($teamReport[$user_id])) {
                         ...
                         $teamReport[$user_id]['total_win'] = round(($teamReport[$user_id]['total_prize'] + $teamReport[$user_id]['total_rebate'] - $teamReport[$user_id]['total_amount']), 4);
                     }
                    //循环加以后是最终的汇总数据
                    if(isset($teamTotalProjects[$user_id])) {
            ...
            $teamTotal['team_total_win'] = round(($teamTotal['team_total_rebate'] + $teamTotal['team_total_prize'] - $teamTotal['team_total_amount']), 4);
            ...
            if (in_array($sortKey, array('total_deposit', 'total_withdraw', 'total_amount', 'total_prize', 'total_win', 'total_rebate'))) {
                $sort_arr = array(); //这个变量用于多维排序
          ...
          //  $curPageTeamReport = array_slice($teamReport, $startPos, DEFAULT_PER_PAGE);
            ...
            self::$view->setVar('userLevelName', $userLevelName);
           // self::$view->setVar('pageList', getPageList($usersNumber));
        }
...
//            $write_cache_flag = $endDate === date('Y-m-d 23:59:59') ? false : true;
            ...
            self::$view->setVar('pageList', getPageList(count($days)));
        }
...
//        $startPos = ($this->request->getGet('curPage', 'intval', 1) - 1) * DEFAULT_PER_PAGE;
        ...
        if ($flag == 1) {
            $users = users::getItems($user['user_id'], true, 0, array(), 8, -1, '', '', '', '', '', '', '', '', '', '', -1,-1,DEFAULT_PER_PAGE,[],'',-1,1);
            unset($users[$user['user_id']]);
            $usersNumber = users::getItemsNumber($user['user_id'], true, 0, 8,-1,'','','','','','','','','','',1,1);
...
//            $write_cache_flag = $endDate === date('Y-m-d 23:59:59') ? false : true;
            ...
            if (($teamTotalProjects = $GLOBALS['mc']->get(__CLASS__, $cacheKey1)) === false)
            {
                ...
                $teamTotalProjects = projects::getChildReport($user['user_id'], $startDate, $endDate,1);
                if ($write_cache_flag)
                {
                    $GLOBALS['mc']->set(__CLASS__, $cacheKey1, $teamTotalProjects, CACHE_EXPIRE_LONG); //放入缓存
                }
            ...
            {
                //>>得到任意用户及其直属下级团队存款量
                $teamTotalDeposits = deposits::getTeamDeposits($user['user_id'], $startDate, $endDate,1);
                if ($write_cache_flag)
                {
                    $GLOBALS['mc']->set(__CLASS__, $cacheKey2, $teamTotalDeposits, CACHE_EXPIRE_LONG); //放入缓存
                }
            ...
            {
                //>>得到任意用户及其直属下级团队提款量
                $teamTotalWithdraws = withdraws::getTeamWithdraws($user['user_id'], $startDate, $endDate,1);
                if ($write_cache_flag)
                {
                    $GLOBALS['mc']->set(__CLASS__, $cacheKey3, $teamTotalWithdraws, CACHE_EXPIRE_LONG); //放入缓存
                }
            ...
            if (($teamTotalCommission = $GLOBALS['mc']->get(__CLASS__, $cacheKey4)) === false)
             {
                 ...
                 $teamTotalCommission = orders::getChildCommission($user, $startDate, $endDate,1);
                 if ($write_cache_flag)
                 {
            ...
            {
                //>>获取团队即时报表
                $teamTotalGifts = userGifts::getTeamTotalGift($user['user_id'], $startDate, $endDate,1);
                if ($write_cache_flag)
                {
