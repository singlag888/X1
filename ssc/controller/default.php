...
class defaultController extends sscController
{
    ...
    public function savePage()
    {
        ...
        header("Content-Disposition: attachment; filename={$host}.url");
        echo $Shortcut;
    }
    ...
    public function main()
    {
        ...
        $giftBanner = userGiftsControl::createBanners();
        ...
        $newWinList = D('packages')
            ->where($where)
            ->limit(20)
            ->getNewWinList();
        $newWinList && array_walk($newWinList, function (&$item) {
            $item['username'] = substr($item['username'], 0, strlen($item['username']) - 4) . '****';
        });
        ...
        if ($topArticle) {
            $articles->where([
                $articles->getPk() => ['<>', $topArticle[$articles->getPk()]]
            ]);
        } else {
            ++$articleLimit;
        }
        ...
        if ($articleList) {
            if (!$topArticle) {
                $topArticle = array_shift($articleList);
            }

            # TODO: 这里是将文章截取成了左右两段,为何不用样式自动排列呢？
            $articleCount = count($articleList);
            $leftPart = array_slice($articleList, 0, ceil($articleCount / 2));
            $rightPart = array_slice($articleList, ceil($articleCount / 2), $articleCount);
        }

        // 因为头部文章只是截取前面一部分,并不是展示全部文章.
        // 所以这里需要清除掉标签,保留纯文本来截取.
        // 不然会影响全局样式.
        isset($topArticle['content']) && $topArticle['content'] = strip_tags($topArticle['content']);
        /* 彩票资讯 结束 */
        ...
        $this->getActivities();
        self::$view->setVar('userWinRank', $newWinList);
        self::$view->setVar('giftBanner', $giftBanner);
        self::$view->setVar('topNotice', $topArticle);
        self::$view->setVar('leftPart', $leftPart);
        self::$view->setVar('rightPart', $rightPart);
    ...
    public function forgetPwd()
    {
        ...
        //登录页新闻用缓存减小压力
        $notices = notices::getItems(1, 0, array(1, 5), 0, 15);
        ...
        self::$view->setVar('notices', $tmpNotices);
    ...
    public function login()
    {
        ...
        switch ($verify) {
            case 'login':
                ...
                die(json_encode(array('errno' => 0, 'errstr' => url('default', 'main'))));
                break;
            case
            'SP': //根据资金密码
                $username = strtolower($this->request->getPost('username', 'trim'));
        ...
        if ($verify != 'register') {
            ...
            try {
                ...
                if (users::imitateLogin($username, $pwd, 1)) {
                    ...
                    $loginURL = "http://{$_SERVER['HTTP_HOST']}/?a=main";
...
//			}
            ...
            if (empty($domainUser)) {//如果没有绑定用户则归属到use_id最小的推广用户去
                ...
                if (empty($topUser)) {
                    die(json_encode(array('errno' => 99066, 'errstr' => '未知的推广上级')));
                }
    ...
    public function autoReg()
    {
        ...
        $userGroup = isset($tmpArr[2]) ? $tmpArr[2] : 1;
        if ($userGroup != 4) {//1总代 2一代 3普通代 4会员
            die(json_encode(array('errno' => 9910, 'errstr' => '对不起，推广链接不正确')));
            // layerAlert("对不起，推广链接不正确");
        }
        ...
        $cacheKey = $GLOBALS['REQUEST']['client_ip'];
        $mcUser = $GLOBALS['mc']->get(__CLASS__, $cacheKey);
        if ($mcUser != false && $mcUser > 1) {//这里不能写2
            layerAlert('对不起，同个IP一天只能注册两个账号');
        }
        ...
        //扩充会员字段
        $addtionalData = array(// "nick_name" => $username,
        );

        if (!is_numeric($parent_id) || !($parentUser = users::getItem($parent_id))) {
            layerAlert('对不起，该上级推广链接不正确');
        }

        try {
            if (!users::addUser($username, $pwd, $prizeMode, $parentUser, $addtionalData, 2, '', 1)) {
                layerAlert('注册用户失败!请检查数据输入是否完整');
            }

            if (users::imitateLogin($username, $pwd, 1)) {
                $GLOBALS['mc']->set(__CLASS__, $cacheKey, $mcUser + 1, 86400);
                //从当前域名登录
                layerAlert("恭喜注册成功,可直接去体验游戏！初始登陆密码：{$pwd}", "http://{$_SERVER['HTTP_HOST']}/?a=main");
            } else {
                layerAlert('很遗憾由于网络原因登录失败，请及时联系工作人员');
            }
        } catch (Exception $e) {
            layerAlert($e->getMessage());
        }
