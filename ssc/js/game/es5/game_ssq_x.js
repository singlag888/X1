...
(function ($) {
    ...
    $.init = function (settings) {
        var KaiQiZhiXuanDanShiWanFa = false;
        //检查传过来的参数的正确性
        var verifyParams = function verifyParams() {
            ...
            if (settings.lotteryId == undefined || !is_numeric(settings.lotteryId) || settings.lotteryId <= 0) {
                ...
                flag = -2;
            } else if (settings.lotteryType == undefined || !is_numeric(settings.lotteryType) || $.inArray(settings.lotteryType, [3]) == -1) {
                flag = -3;
        ...
        function todayGetCurIssue(lastIssueInfo) {
            ...
            $("#todayIssuesBody").prepend('<ul class="fix"><li class="w120iss">' + lastIssueInfo.issue + '</li><li class="min150ciss"><b>' + codes2.split(' ').join('</b><b>') + '</b></li></ul>');
        }
        var helper = {
            ...
            showPirze: function showPirze() {
                if ($.isEmptyObject(ps.curMethod) !== true) {
                    ...
                    $.each(ps.rebateGapList, function (k, v) {
                        ...
                        //$('#curPrizeSpan').append('<option value="' + k + '">' + selectPrize + '</option>');
                    ...
                    $('#selectRebate').slider({ //初始化滑动块
                        range: $('#curPrizeSpan option').length - 1,
                        min: 0,
        ...
        var methodBar = {
            ...
            selectDefault: function selectDefault() {
                $("#methodGroupContainer").find("label:contains('普通')").click();
                $(".playNav ul ul").hide();
            }
        };
        ...
        var ballBar = {
            ...
            showInput: function showInput() {
                ...
                var str = '<div class="manualInput fix"><div class="manualInputTop"><ul><li><p class="NumbBasket">号码篮</p><textarea cols="" rows="" class="inputTA MachineSeleBg" id="inputTA"></textarea></li><li id="delTA" class="delTA custBtnStyle">清空号码</li></div><div class="manualInputBottom"><li class="inputTip1"><p>提示：</p><p>1) 仅支持单式；</p><p>2) 每行一个投注号；</p><p>3) 每行有五个红球和一个蓝球，每个球必须是两位数字，红球和蓝球之间以一个":"（冒号）分隔；</p><p>4) 红球与红球之间或蓝球与蓝球之间用一个空格间隔；</p><p>4) 例如：01 02 03 04 05:01</p></li></ul></div></div>';
        ...
        var buyBar = {
            ...
            selectCodeBtn_Click: function selectCodeBtn_Click() {
                ...
                if ($("#inputTA").length > 0) {
                    ...
                    var arrs = str.split(/\n/);
                    var re = /^(([012]\d)|(3[0123]))(\s(([012]\d)|(3[0123]))){4}:((0[1-9])|(1[0-6]))$/;
                    var arr = tool.uniqueArr(arrs); //号码去重
                    var tmp = tmp1 = '';
                    for (var i in arr) {
                        if (!re.test(arr[i])) {
                            parent.layer.alert("您输入的号码" + arr[i] + "有误，请重新检查输入", { icon: 7 });
                            return false;
                        }
                        tmp = arr[i].replace(/\s/g, '_');
                        tmp1 = tmp.replace(/:/g, ',');
                        allCodes.push(tmp1);
                    }
                    ...
                    if (allCodes.length <= 1000) {
                        for (var i in allCodes) {
                            ...
                            $(strPart1 + strPart2 + allCodes[i] + strPart3).appendTo("#projectList");
                        }
                    } else {
                        var allCodesStr = '';
                        var allProjectsStr = '';
        ...
        var traceFunc = {
            ...
            confirmTraceBtn_Click: function confirmTraceBtn_Click() {
                ...
                traceFunc.destroyTracePage();parent.layer.confirm(confirmInfo, { icon: 7 }, function (i) {
                    $.post("?c=game&a=play", {
                        op: "buy",
                        lotteryId: ps.lotteryId,
                        issue: ps.curIssueInfo.issue,
        ...
        var drawBar = {
                ...
                if (ps.lastIssueInfo.code == "") {
                    ...
                    if (ps.lastIssueInfo.issue != latest.issue) {
                        ...
                        if ($("#todayDrawBtn").hasClass("cur")) {
                            ...
                            var str = '<ul class="fix"><li class="w120iss">' + v.issue + '</li><li class="min150ciss">' + v.code + "</li>";
                ...
                if (nums != null) {
                    $.each(nums, function (i, n) {
                        if (i < numlength - 1) {
                            ...
                            $('#thisIssueNumUL').append('<span class="lan pendingBall"></span>');
                        }
                    });
                }
                            ...
                            if (response.errno == 0) {
                                ...
                                if (response.kTime != 0) {
                                    ...
                                    if (typeof response.issueInfo.code != "undefined") {
                                        ...
                                        if ($("#todayDrawBtn").hasClass("cur")) {
                                            var v = ps.todayDrawList[0];
                                            var str = '<ul><li class="w120iss">' + v.issue + '</li><li class="min150ciss">' + v.code + "</li>";
            ...
            todayDrawBtn_Click: function todayDrawBtn_Click() {
                $("#todayIssuesHead").html('<li class="w120iss">期号</li><li class="min150ciss">开奖号</li>');
                ...
                $.each(ps.todayDrawList, function (k, v) {
                    ...
                    var str = '<ul class="fix1"><li class="w120iss">' + v.issue + '</li><li class="min150ciss"><b>' + arr + "</b></li></ul>";
        ...
        function remainTime(d) {
            //console.log(d);
            //var day = d.day.toString().split('');
            var hour = d.hour.toString().split('');
            var minute = d.minute.toString().split('');
            var second = d.second.toString().split('');
            //console.log(d);
            $("#thisIssueRemainTime").html("<span>" + d.day + "</span><em>:</em>" + "<span>" + hour[0] + "</span><span>" + hour[1] + "</span><em>:</em>" + "<span>" + minute[0] + "</span><span>" + minute[1] + "</span><em>:</em>" + "<span>" + second[0] + "</span><span>" + second[1] + "</span>");
        }
            ...
            switch (ps.curMethod.name) {
                ...
                case 'HLZX':
                    ...
                    if (Object.prototype.toString.call(codes) == '[object Array]') {
                        //复试
                        part1 = codes[0].split('_');
                        part2 = codes[1].split('_');
                    ...
                    } else if (/^(([012]\d)|(3[0123]))(_(([012]\d)|(3[0123]))){4},((0[1-9])|(1[0-6]))$/.test(codes)) {
                        //单式
                        singleNum = 1;
                        isDup = singleNum > 1 ? 1 : 0;
                    }
