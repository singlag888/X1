...
(function ($) {
    ...
    $.init = function (settings) {
        ...
        function todayGetCurIssue(lastIssueInfo) {
            ...
            var codes = "";
            var codes2 = "";
            for (var i = 0, summarys = 0; i < lastIssueInfo.code.length; i++) {
                codes += "<b>" + lastIssueInfo.code[i] + "</b>";
                codes2 += lastIssueInfo.code[i];
                summarys += parseFloat(lastIssueInfo.code[i]);
            }
            $("#todayIssuesBody").prepend('<ul class="fix"><li>' + lastIssueInfo.issue + '</li><li id="numshezhi">' + codes + '<span>=</span><a class="numshezhia">' + summarys + '</a></li></ul>');
        }
            ...
            showPirze: function () {
                if ($.isEmptyObject(ps.curMethod) !== true) {
                    ...
                    $('#selectRebate').slider({//初始化滑动块
                     ...
                     if(ps.curMethod.name==='XYDXDS'||ps.curMethod.name==='JDX'||ps.curMethod.name==='XYSB'){
        ...
        var ballBar = {
            ...
            generateBall: function () {
                ...
                /* 信用不需要最下边的输入框  begin */
                if ($.inArray(ps.curMethod.name, ['JDX', 'XYDXDS', 'XYSB']) !== -1) {
                    ...
                    $.each(numList, function (ii, nn) {
                        switch (ps.curMethod.name) {
                            ...
                            case 'XYSB':
                                ...
                                ballStr += '<li class="list_1">' + '<span class="ball_hz">' + nn + '</span>' +'<b class="rebateValue"></b>'+ '<input type="number" value="" list="itemlist" name="multiple" class="moneyNum multiple1" maxlength="5" data-ball="' + nn + '" /></li>';
                                break;
                            default :
                                ballStr += '<li>' + nn + "</li>";
                        }
...
//                        }
            ...
            confirmBtn_Click: function (fastBet) {
                ...
                if (
                    ...
                    betTotalAmount = betTotalBetCount*totalSingleInfo
                ...
                var $postData = {
                    ...
                    multiple: $op === 'xbuy' ? 1 :$("#totalSingleInfo input").val(),
                    token: $("#token").val()
                };
...
//                $("#ui-widget-overlay2").remove();
        ...
        var drawBar = {
            init: function () {
                ...
                $('#thisIssueNumUL').append('<span class="black">=</span>');
                $('#thisIssueNumUL').append('<span></span>');
                    ...
                    var openCodeCallback = function () {
                        ...
                        if ($("#thisIssueNumUL").children(".pendingBall").length > 0) {
                            ...
                            window.setTimeout(function () {
                                ...
                                if (index == numlength) {
                                    $("#thisIssueNumUL span:last").text(hezhi);
                                    return;
                                }
                                openCodeCallback();
                            }, 500);
            ...
            todayDrawBtn_Click: function () {
                ...
                $.each(ps.todayDrawList,
                    function (k, v) {
                        ...
                        var splitCode2 = "";
                        var summary = "";
                        for (var i = 0, summary = 0; i < v.code.length; i++) {
                            splitCode += "<b>" + v.code[i] + "</b>";
                            splitCode2 += v.code[i];
                            summary += parseFloat(v.code[i]);
                        }

                        var str = '<ul class="fix"><li>' + v.issue + '</li><li>' + splitCode + '<span>=</span><a class="numshezhia">' + summary + '</a></li></ul>';
                        //str +="</ul>";
