...
$aMenus = [
    ...
    'game_traceList'   => '追号记录',
    'user_childList'   => '团队管理',
    // 'egame_gameRecodeList'  => '电子游戏',
    'user_receiveBox'  => '站内信箱'
];
...
//        'game_packageTeamList' => '团队投注明细',
        ...
        'user_teamNowReport' => '团队实时报表',
//        'user_teamDayReport' => '团队日结报表',
        'user_teamWinReport' => '团队盈亏报表',
        'fin_teamOrderList' => '团队帐变报表',
        'user_childReport' => '团队汇总报表',
        'user_childList' => '会员管理',
        'user_regChild' => '新增会员',
    ],
    'egame_gameRecodeList' => [
        'egame_gameRecodeList'=> '游戏记录',
        'egame_transferRecodeList' => '转账记录',
    ],
];
...
foreach ($aSubMenus as $parentRouteName => $aMenu)
{
    if(in_array($routeName, array_keys($aMenu)))
    {
        foreach ($aMenu as $route => $name)
        {
            ...
            //todo 添加点击样式
            $subHtml .= '<a href="?c='.$c.'&a='.$a .'" class="operate_submenu_btn'.( $route == $routeName? ' active': '').'">'.$name.'<span>&gt;<i></i>&lt;</span></a>';
        }
...
foreach ($aMenus as $route => $name)
{
    list($c, $a) = explode('_', $route);
    //todo 添加点击样式
    $html .= '<a href="?c='.$c.'&a='.$a .'" class="user_menu'.$flag.' deTop_common_btn'.( $route == $routeName? ' active': '').'"><span>'.$name.'</span><i><b>&laquo;</b></i></a>';
    $flag++;
}
$html .= '</div>';
